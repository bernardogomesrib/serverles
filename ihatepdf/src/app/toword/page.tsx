...
import { toWord } from '@/util/lambdaControll';
...
export default function MergePdfPage() {
        ...
        try {
            const filesPayload = await Promise.all(
                files.map(f => new Promise<{ pdf: string, rotation: number }>((resolve, reject) => {
                    ...
                    reader.onload = () => resolve({ pdf: (reader.result as string).split(',')[1], rotation: 0 });
                    reader.onerror = reject;
                    reader.readAsDataURL(f.file);
                }))
            );
            const result = await toWord(filesPayload, ocrEnabled);
            if (result.url) {
                setMergedUrl(result.url);
                setUiState('completed');
            } else {
                throw new Error(result.error || "Falha ao converter PDFs.");
            }
        } catch (error) {
            console.error(error);
    ...
    return (
        <div className="w-full bg-white flex flex-col items-center justify-center p-4 sm:p-10 text-center min-h-[80vh]">
            ...
            {uiState === 'initial' && (
                <div>
                    <h1 className="text-4xl font-bold text-gray-800">Transformar arquivos PDF em Word</h1>
                    <p className="text-xl text-gray-500 mt-4 max-w-2xl">Transforme os seus arquivos .docx em PDF</p>
                ...
                <div className="w-full h-full flex flex-col">
                        ...
                        {files.map((pdf, index) => (
                            <div key={pdf.id} draggable onDragStart={() => handleDragStart(index)} onDragEnter={() => handleDragEnter(index)}
                                ...
                                <div className="w-full h-full flex items-center justify-center overflow-hidden rounded-md bg-gray-100">
                                    {pdf.thumbnailUrl ? (
                                        <img src={pdf.thumbnailUrl} alt={`Miniatura de ${pdf.file.name}`} className="w-full h-full object-contain" />
                                    ) : (
                    ...
                    <div className="flex-shrink-0 w-full bg-white p-4 border-t sticky bottom-0">
                        <div className="max-w-7xl mx-auto flex justify-end">
                            {uiState === 'filesSelected' && (
                                <div className="flex flex-col items-end gap-2 align-center justify-center">
                                    ...
                                    <div className="flex items-center gap-4 mb-2">
                                        <label className="flex items-center gap-1 cursor-pointer">
                                            ...
                                            />
                                            <span className="text-gray-700">Sem OCR</span>
                                        </label>
                                        <label className="flex items-center gap-1 cursor-pointer">
                                            <input
                                                type="radio"
                                                ...
                                                className="accent-red-600"
                                            />
                                            <span className="text-gray-700">Com OCR</span>
                                        </label>
                                    </div>
                                    <button
                                        onClick={handleMerge}
                                        className="bg-red-600 text-white font-bold text-lg px-8 py-4 rounded-lg hover:bg-red-700 disabled:opacity-50"
                                    >
                                        Converter
                                    </button>
                                </div>
